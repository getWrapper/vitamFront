...
(function() {
	App.controller('vendorController', ['$scope', '$http', 'Flash', 'MessagesService', 'FormUtilService', 'PaginatorConfigService', 'VendorService', function ($scope, $http, Flash, MessagesService, FormUtilService, PaginatorConfigService, VendorService) {
		$scope.vendor = getExampleData();
		$scope.search;
		$scope.form = {};
		$scope.form.code;
	    ...
	    $scope.showAdd = function () {
			...
			FormUtilService.resetFormValidations($scope.vendorForm);
	    };
	    ...
	    $scope.save = function (valid) {
			if (valid) {
				$scope.addUpdate = false;
				if ($scope.form.isAdd) {
					VendorService.create($scope);
				} else {
					VendorService.update($scope);
				}
			}
	    };
	    ...
	    $scope.remove = function (world) {
			VendorService.remove(world, $scope);
	    };
	    $scope.activate = function (world) {
			VendorService.activate(world, $scope);
	    };
